...
class PerceiverIO(nn.Module):
    ...
    def forward(self, user_ids, item_ids):
        ...
        residual_pred = self.output_head(decoded).squeeze(-1).squeeze(1)
...
with torch.no_grad():
    ...
    print(f"  Mean: {residual_mean.item():.4f}")
    print(f"  Std:  {residual_std.item():.4f}")
    print(f"  Min:  {residual_target.min().item():.4f}")
    print(f"  Max:  {residual_target.max().item():.4f}")
...
print("="*60)
print(f"{'Model':<35} {'RMSE':<10} {'Improvement vs Constant':<25}")
print("-" * 70)

print(f"{'Constant Mean Baseline':<35} {constant_rmse:<10.4f} {'-':<25}")
print("-" * 70)

for k in k_values:
    pmf_model, rmse = pmf_results[k]
    improvement = ((constant_rmse - rmse) / constant_rmse) * 100
    marker = " ★" if k == best_k else ""
    print(f"{'PMF (k=' + str(k) + ')' + marker:<35} {rmse:<10.4f} {improvement:>6.2f}%")

print("-" * 70)

perceiver_improvement = ((constant_rmse - final_rmse) / constant_rmse) * 100
print(f"{'Pure Perceiver IO':<35} {final_rmse:<10.4f} {perceiver_improvement:>6.2f}%")
print("="*70)
print("\n" + "="*60)
print("DETAILED COMPARISON")
print("="*60)
print(f"Constant Mean Baseline:           RMSE = {constant_rmse:.4f}")
print(f"Best PMF (k={best_k}):                 RMSE = {best_pmf_rmse:.4f}")
print(f"Pure Perceiver IO:                RMSE = {final_rmse:.4f}")
print("-" * 60)
print(f"Improvement over Constant:        {(constant_rmse - final_rmse):.4f} ({perceiver_improvement:.2f}%)")
print(f"Improvement over Best PMF:        {(best_pmf_rmse - final_rmse):.4f} ({((best_pmf_rmse - final_rmse) / best_pmf_rmse * 100):.2f}%)")
print("="*60)
